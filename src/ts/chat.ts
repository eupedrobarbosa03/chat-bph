...
class Chat {
    ...
    private botCommands(text: string) {
        ...
        switch (found[0]) {
            ...
            case "+ensinamentos":
                ...
                this.pendingMessages.push(
                    newListTeaching,
                    `Total de ensinamentos: ${teachings.length}.`);
                break;
            case "+ajuda":
                this.pendingMessages.push(
                    `Você usou o comando <strong>+ajuda</strong>.`,
                    `Essa ajuda é para você entender o sistema de ensinamentos.`,
                    `<strong>Tutorial:</strong> Seu ensinamento deve possuir dois identificadores, o #. Há dois modelos de ensinamentos.`,
                    `<strong>Modelo Simples: </strong> #título do ensinamento# e #ensinamentos#`,
                    `<strong>Modelo Avançado:</strong> #título do ensinamento# e #ensinamento-ensinamento-ensinamento#`,
                    `A grande diferença é que no modelo simples não há separadores (-), e no modelo avançado possuem um ou mais.`,
                    `<strong>Exemplo modelos simples:</strong> chat, quando eu falar #teste# você deve falar #pedro é um grande amigo.#`,
                    `<strong>Exemplo modelos avançado</strong>: chat, quando eu falar #teste# você diz #teste-teste-teste#.`,
                    `Aceito letras, números, espaços, ponto (.), vírgula (,), interrogação e exclamação.`
                );
                break;
            case "+projetos":
                this.pendingMessages.push(
    ...
    private showTeachings(text: string) {
        const storangeTeachings: Teaching[] = storage.list();
        const existingTeaching = storangeTeachings.find((teaching) => 
            teaching.title === text.toLowerCase().trim());
        if (!existingTeaching) return false;
